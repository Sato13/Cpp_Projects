...
int main()
{
	...
	string testfile = "/home/shared/benton_class/Data/Iris/Iris.test";
	string train = "/home/shared/benton_class/Data/Iris/Iris.train";
	string line;
	size_t index = 0;
	string value;
		...
		for (int i = start; i <= finish; i++) {
			...
			for (int j = 0; j <= tst_size; j++) {
				...
				for (int z = 0; z <= curTrain.size() - 1; z++) {
					for (int e1 = 0; e1 <= trainV.size() - 1; e1++) {
						dist += pow(abs(curTrain[e1] - current_test[e1]), 2);
					}
					dist = sqrt(dist);
				}
